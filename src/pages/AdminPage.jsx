...
export default function AdminPage() {
    ...
    const savePublishDate = async () => {
        ...
        setMsg("");
        try {
            // Try to list existing settings
            const existing = await databases.listDocuments(databaseId, COLLECTIONS.settings);
            if (existing.total > 0) {
                await databases.updateDocument(databaseId, COLLECTIONS.settings, existing.documents[0].$id, {
                    resultPublishDate: new Date(publishDate).toISOString(),
                });
            } else {
                await databases.createDocument(databaseId, COLLECTIONS.settings, ID.unique(), {
                    resultPublishDate: new Date(publishDate).toISOString(),
                });
            }
